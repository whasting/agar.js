...
class ComputerPlayer {
  ...
  senseCells(playerList, foodStuff) {
    let radius = this.cell.realMass;
    ...
    playerList.forEach(player => {
      if (player.id !== this.id) {
          ...
          player.cell.realMass > this.cell.realMass)
        {
          // RUN
          this.chaseF = false;
          this.run(player, player.x, player.y);
        } else if
          (distance < 350 + this.cell.realMass / 2 &&
          distance > radius &&
          player.cell.realMass < this.cell.realMass)
  ...
  run(player, otherX, otherY) {
    ...
    this.x += maxDistanceCoords[0] * this.step;
    this.y += maxDistanceCoords[1] * this.step;
  ...
  chase(target) {
    ...
    this.x += minDistanceCoords[0] * this.step;
    this.y += minDistanceCoords[1] * this.step;
