...
class HumanPlayer {
  constructor(canvasW, canvasH, x, y) {
    ...
    // this.playerCell.realMass * 2 = this.playerCell.realMass * 2;
    // this.playerCell.realMass * 2 = this.playerCell.realMass * 2;
  }

  senseFood(foodStuff) {
    // playerCell.mass returns the radius of the cell
    let radius = this.playerCell.realMass;
  ...
  update(step, worldW, worldH, controls) {
    ...
    if (this.x - this.playerCell.realMass * 2 / 2 < 2500) {
      this.x = this.playerCell.realMass * 2 / 2 + 2500;
    } else if (this.x + this.playerCell.realMass * 2 / 2 > worldW) {
      this.x = worldW - this.playerCell.realMass * 2 / 2;
    }

    if (this.y - this.playerCell.realMass * 2 / 2 < 2500) {
      this.y = this.playerCell.realMass * 2 / 2 + 2500;
    } else if (this.y + this.playerCell.realMass * 2 / 2 > worldH) {
      this.y = worldH - this.playerCell.realMass * 2 / 2;
    }
  }
